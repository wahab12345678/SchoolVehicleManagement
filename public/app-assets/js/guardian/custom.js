      ...
      var dt_basic = dt_basic_table.DataTable({
        ...
        columnDefs: [
            ...
            },
            {
                targets: 2, // Image
                render: function (data, type, full, meta) {
                  return full.img_path ? `<img src="${full.img_path}" alt="Guardian Image" width="50" height="50">` : ''; // Render image or empty if not available
                }
              },
            {
            ...
            },
            {
                ...
                render: function (data, type, full, meta) {
                    ...
                    //             <li><a class="dropdown-item delete-guardian" href="javascript:void(0);" data-id="${full.id}">Delete</a></li>
                    ...
                    return (
                      ...
                      '<a href="javascript:;" class="dropdown-item toggle-visible-status" data-id="' + full.id + '" data-status="' + full.is_visible + '">' +
                        ...
                        '</a>' +
                      '<a href="javascript:;" class="dropdown-item delete-guardian" data-id="' + full.id + '">' +
        ...
        buttons: [
          {
            text: feather.icons['plus'].toSvg({ class: 'me-50 font-small-4' }) + 'Add New Guardian',
...
//   // Populate the modal form with the guardian data
//   $('#edit-guardian-name').val(name);
//   $('#edit-guardian-description').val(description);

//   // Set the status radio button based on the guardian's status
//   if (status == 'Active')
//   {
...
//   // Set the form's hidden guardian ID field
...
$(document).on('click', '.modal-slide-in-edit', function () {
  ...
  $.ajax({
      ...
      success: function (response)
      {
        ...
        $('#edit-guardian-name').val(response.guardian.name);
        $('#edit-guardian-description').val(response.guardian.description);

        let keyPoints = response.guardian.key_points;
        ...
        $('#edit-guardian-id').val(response.guardian.id);
        $('#modals-slide-in-edit').modal('show');
      },
...
$('#update-guardian').on('submit', function (e) {
  e.preventDefault();
...
$(document).on('click', '.delete-guardian', function () {
  const guardianId = $(this).data('id');  // Get guardian ID from the button data attribute

  // Show confirmation prompt before deletion
  if (confirm('Are you sure you want to delete this guardian?')) {

      // Send the AJAX request to delete the guardian
      $.ajax({
          url: `/guardian/categories/${guardianId}`,  // Correct URL with guardian prefix
          ...
          error: function (error) {
              ...
              alert('There was an error deleting the guardian.');
          }
      });
  }
});
...
//  $('.datatables-basic tbody').on('click', '.toggle-status', function ()
 $(document).on('click', '.toggle-status', function ()
 {
  ...
  const id     = $(this).data('id');  // Get guardian ID from the button data attribute
  const status = $(this).data('status');

  if (confirm('Are you sure you want to change the status of this record?')) {
      // Send the AJAX request
      $.ajax({
          url: `/guardian/change-status`,  // Correct URL with guardian prefix
...
$(document).on('click', '.toggle-visible-status', function ()
{
 const id     = $(this).data('id');  // Get guardian ID from the button data attribute
 const status = $(this).data('status');
 if (confirm('Are you sure you want to show this guardian to Frontend?')) {
     // Send the AJAX request
     $.ajax({
         url: `/guardian/change-visibility`,  // Correct URL with guardian prefix
         type: 'POST', // HTTP method
         contentType: 'application/json', // Specify JSON format
         headers: {
             'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') // Include CSRF token for Laravel
