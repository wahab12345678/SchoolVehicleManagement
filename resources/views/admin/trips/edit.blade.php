        ...
        <div class="content-header row">
            <div class="content-header-left col-md-9 col-12 mb-2">
                <div class="row breadcrumbs-top">
                    <div class="col-12">
                        <h2 class="content-header-title float-start mb-0">Edit Trip</h2>
                ...
                <div class="row">
                    <div class="col-md-12 col-12">
                        <div class="card">
                            ...
                            <div class="card-body">
                                <form class="form form-horizontal" method="POST" action="{{ route('admin.trips.update', $trip->id) }}">
                                    @csrf
                                    @method('PUT')
                                    <div class="row">
                                        <div class="col-12">
                                                ...
                                                <div class="col-sm-9">
                                                    <select id="student_id" name="student_id" class="form-select @error('student_id') is-invalid @enderror" required>
                                                        ...
                                                        @foreach($students as $student)
                                                            <option value="{{ $student->id }}" {{ old('student_id', $trip->student_id) == $student->id ? 'selected' : '' }}>{{ $student->name }} ({{ $student->class ?? 'No Class' }})</option>
                                                        @endforeach
                                                    </select>
                                                    @error('student_id')
                                                ...
                                                <div class="col-sm-9">
                                                    <select id="vehicle_id" name="vehicle_id" class="form-select @error('vehicle_id') is-invalid @enderror" required>
                                                        ...
                                                        @foreach($vehicles as $vehicle)
                                                            <option value="{{ $vehicle->id }}" {{ old('vehicle_id', $trip->vehicle_id) == $vehicle->id ? 'selected' : '' }}>{{ $vehicle->number_plate }} - {{ $vehicle->model }} ({{ ucfirst($vehicle->type) }})</option>
                                                        @endforeach
                                                    </select>
                                                    @error('vehicle_id')
                                                        <div class="invalid-feedback">{{ $message }}</div>
                                                ...
                                                <div class="col-sm-9">
                                                    <select id="route_id" name="route_id" class="form-select @error('route_id') is-invalid @enderror" required>
                                                        ...
                                                        @foreach($routes as $route)
                                                            <option value="{{ $route->id }}" {{ old('route_id', $trip->route_id) == $route->id ? 'selected' : '' }}>{{ $route->name }}</option>
                                                        @endforeach
                                                    </select>
                                                    @error('route_id')
                                                        <div class="invalid-feedback">{{ $message }}</div>
                                            ...
                                            <div class="mb-1 row">
                                                ...
                                                <div class="col-sm-9">
                                                    <select id="status" name="status" class="form-select @error('status') is-invalid @enderror" required>
                                                        ...
                                                        <option value="en_route" {{ old('status', $trip->status) == 'en_route' ? 'selected' : '' }}>En Route (On the way)</option>
                                                        <option value="arrived" {{ old('status', $trip->status) == 'arrived' ? 'selected' : '' }}>Arrived</option>
